...
class HomeRenderer:
    ...
    def render_skeleton_ui(self):
        ...
        for container in [chart1_container, chart2_container, chart3_container, chart4_container, chart5_container]:
            with container:
...
def render():
    ...
    ui.timer(0.1, lambda: renderer.render_chart(containers['chart1_container'], 'net_worth', 'Net Worth'), once=True)
    ui.timer(0.1, lambda: renderer.render_chart(containers['chart2_container'], 'asset_vs_liabilities', 'Asset vs Liabilities'), once=True)
    ui.timer(0.1, lambda: renderer.render_chart(containers['chart3_container'], 'cash_flow', 'Cash Flow'), once=True)
    ui.timer(0.1, lambda: renderer.render_chart(containers['chart4_container'], 'avg_expenses', 'Avg Expenses'), once=True)
    ui.timer(0.1, lambda: renderer.render_chart(containers['chart5_container'], 'income_vs_expenses', 'Income vs Expenses'), once=True)
