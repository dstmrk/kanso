...
def render_currency_selector() -> None:
    ...
    with ui.element('div').classes('dropdown'):
        ...
        dropdown_btn = ui.element('div').props('tabindex="0" role="button"').classes('btn btn-outline w-48')
        with dropdown_btn:
            label_display = ui.label(current_label).classes('mx-auto')

        # Dropdown menu
        with ui.element('ul').props('tabindex="0"').classes('dropdown-content menu bg-base-100 rounded-box z-[1] w-48 p-2 shadow mt-2'):
            for code, label in CURRENCY_OPTIONS.items():
                with ui.element('li'):
                    def make_click_handler(c, l):
                        def handler():
                            save_currency_preference(type('Event', (), {'value': c})())
                            label_display.set_text(l)
...
def render_refresh_button() -> None:
    ...
    with ui.element('button').on('click', refresh_data).classes('btn bg-secondary hover:bg-secondary/80 text-secondary-content w-48'):
        ui.label('Refresh Data')


def render_logout_button() -> None:
    """Render logout button."""
    with ui.element('button').on('click', lambda: ui.navigate.to(pages.LOGOUT_PAGE)).classes('btn btn-outline btn-error w-48'):
        ui.label('Logout')
