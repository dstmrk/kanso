...
class TestParseMonetaryValue:
    ...
    def test_parse_european_format_eur(self):
        ...
        assert parse_monetary_value("€ 999,99") == 999.99
    ...
    def test_parse_european_format_chf(self):
        ...
        assert parse_monetary_value("CHF 1000") == 1000.0
        assert parse_monetary_value("Fr 1.234,56") == 1234.56  # Dot as thousands separator
    ...
    def test_parse_japanese_format_jpy(self):
        ...
        assert parse_monetary_value("JPY 1234") == 1234.0
    ...
    def test_parse_plain_number_no_separator(self):
        ...
        assert parse_monetary_value("1234") == 1234.0
        assert parse_monetary_value("5000") == 5000.0
    ...
    def test_parse_with_currency_override(self):
        ...
        assert parse_monetary_value("1234.56", currency="USD") == 1234.56
    ...
    def test_parse_text_only(self):
        ...
        assert parse_monetary_value("Total") == 0.0

    def test_parse_invalid_mixed_format(self):
        """Test parsing invalid mixed formats returns 0.0."""
        assert parse_monetary_value("€€€") == 0.0
        assert parse_monetary_value("abc123xyz") == 0.0

    # Large number tests
    def test_parse_large_numbers(self):
        """Test parsing large monetary values."""
        assert parse_monetary_value("€ 1.234.567,89") == 1234567.89
        assert parse_monetary_value("$1,234,567.89") == 1234567.89
        assert parse_monetary_value("¥12,345,678") == 12345678.0
    ...
    def test_parse_ambiguous_format(self):
        ...
        assert parse_monetary_value("€ 1.234") == 1234.0  # dot = thousands
        # With USD symbol, should use US format
        assert parse_monetary_value("$ 1,234") == 1234.0  # comma = thousands

    # Zero values
    def test_parse_zero_values(self):
        """Test parsing various representations of zero."""
        assert parse_monetary_value("€ 0") == 0.0
        assert parse_monetary_value("€ 0,00") == 0.0
        assert parse_monetary_value("$0.00") == 0.0
        assert parse_monetary_value("0") == 0.0
...
class TestParseMonetaryValueEdgeCases:
    ...
    def test_parse_very_small_decimals(self):
        ...
        assert parse_monetary_value("€ 0,01") == 0.01
        assert parse_monetary_value("$0.01") == 0.01

    def test_parse_no_whole_part(self):
        """Test parsing values with no whole number part."""
        assert parse_monetary_value("€ ,50") == 0.50
        assert parse_monetary_value("$.50") == 0.50

    def test_parse_trailing_zeros(self):
        """Test parsing with trailing zeros."""
        assert parse_monetary_value("€ 100,00") == 100.0
        assert parse_monetary_value("$100.00") == 100.0

    def test_parse_leading_zeros(self):
        """Test parsing with leading zeros."""
        assert parse_monetary_value("€ 0.100") == 100.0  # European: dot = thousands
        assert parse_monetary_value("$0001234.56") == 1234.56
    ...
    def test_parse_multiple_currency_symbols(self):
        ...
        assert parse_monetary_value("€ $ 100") == 100.0
